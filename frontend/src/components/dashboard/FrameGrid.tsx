...
const FrameGrid: React.FC<FrameGridProps> = ({
  ...
  const frames = useMemo(() => {
    ...
    for (let i = 0; i < displayTotal; i++) {
        ...
        if (renderedSet.has(frameNum)) status = 'done';
        else if (failedSet.has(frameNum)) status = 'failed';
        else if (assignedSet.has(frameNum)) status = 'processing';
        
        items.push({ num: frameNum, status });
    }
    return items;
  }, [displayTotal, renderedFrames, failedFrames, assignedFrames, startFrame]);
        ...
        {frames.map((frame) => (
          ...
          >
            <motion.div
              layout
              animate={{
                scale: frame.status === 'processing' ? [1, 1.15, 1] : 1,
                backgroundColor: 
              ...
              className="w-[18px] h-[18px] rounded-[3px] cursor-crosshair border border-white/5 relative z-10"
            />
