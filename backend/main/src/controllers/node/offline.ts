...
export const checkAndUpdateOfflineNodes = async (): Promise<void> => {
  try {
    ...
    if (offlineNodes.length > 0) {
      ...
      // ── SELF-HEALING: Also sweep any "active" BullMQ jobs that might be orphaned ──
      const recoveredCount = await forceRequeueActiveJobs();
      if (recoveredCount > 0) {
        console.log(`🛡️  Self-healing: Recovered ${recoveredCount} orphaned active jobs from offline nodes`);
        if (wsService) {
          await wsService.notifyNodesToCheckJobs();
        }
      }
